...
## 1. 요구사항
>- 1. 시작화면, 게임화면 총 2개의 화면(씬)이 존재
>- 2. 시작화면에는 게임 시작, 이어 하기, 설정, 게임 종료 버튼이 존재
>- 3. 게임 시작 클릭 시 게임화면으로 이동
>- 4. 이어 하기 클릭 시 이전에 플레이하던 게임화면으로 이동
>- 5. 설정 화면에서는 게임의 각종 설정이 존재
>- 6. 음량 조절과 게임 종료 버튼이 있으며, 좌상단 X 버튼을 눌러 이전화면으로 복귀
>- 7. 게임화면에는 플레이 시간, 현재 높이가 상단에 표시
>- 8. 조작은 WASD로 전후좌우 이동, Space로 점프, Esc로 설정 화면 및 게임 종료 버튼 UI
>- 9. 게임의 종료 조건은 2장 마지막의 목표에 도달할 시
>- 10. 마지막 목표 도달 시, 애니메이션이 재생됨
>- 11. 애니메이션 종료 후, 플레이 시간과 높이, 클리어 횟수가 기록된 석판 UI가 나옴
>- 12. 돌아가기 버튼을 눌러 초기화면으로 복귀
<br>
